...
const goSideWays = () => {
    const conf1 = {
        TRAILING_BUY_AMOUNT: 0.4,
        TRAILING_STOP_AMOUNT: 0.2,
        SYMBOL: 'NVDA',
        QUANTITY: 10,
        TIME_SLEEP: 2.5,
        TIME_LAPSE: 60 * 2,
        RESET_SELL_PRICE_EVERY: 4,
        LIVE: true
    }
    run(conf1)
    ...
    while (true) {
        ...
        if (state.soldOut) {
            ...
            if (state.elapsedTimeAfterSellShort > TIME_LAPSE || state.elapsedTimeAfterSellLong > TIME_LAPSE) {
                updateStateAfterMisBuyLong(state, TRAILING_BUY_AMOUNT, RESET_SELL_PRICE_EVERY)
                updateStateAfterMisBuyShort(state, TRAILING_BUY_AMOUNT, RESET_SELL_PRICE_EVERY)
                continue
            }

            if (state.currentPrice >= state.trailingBuyPriceLong) {
                if (state.currentPrice >= state.sellPrice) {
                    ...
                    logger.info(`Price has risen the trailing buy price BUT ${state.currentPrice} is below ${state.sellPrice}. Resetting to current price`)
                }

            } else if (state.currentPrice <= state.trailingBuyPriceShort) { 
                if (state.currentPrice <= state.sellPrice) {
                        
                    logger.info("Price has fallen trailing SHORT buy price and elapsed time is within the allowed range, placing buy order.")
                    const sellShortResponse = await placeOrder('SELL_SHORT', SYMBOL, QUANTITY, LIVE)
                    ...
                    logger.info(`Price has fallen the trailing buy price BUT ${state.currentPrice} is below ${state.sellPrice}. Resetting to current price`)
                }
            } else  {
            ...
            if (state.buyType === BUY_TYPE.LONG) {
                ...
                if (state.currentPrice <= state.trailingSellPriceLong || state.currentPrice >= state.buyPrice + TRAILING_STOP_AMOUNT) { 
                ...
                if (state.currentPrice >= state.trailingSellPriceShort || state.currentPrice <= state.buyPrice - TRAILING_STOP_AMOUNT) { 

                    logger.info("Price has rise to the trailing sell price, selling now.")
                    const sellResponse = await placeOrder('BUY_TO_COVER', SYMBOL, QUANTITY, LIVE)
...
const updateStateToReadyToBuyLong = (state, TRAILING_BUY_AMOUNT) => {
    ...
    state.trailingBuyPriceLong = Number(state.lowestPriceAfterSell + TRAILING_BUY_AMOUNT)
}
...
const updateStateAfterMisBuyLong = (state, TRAILING_BUY_AMOUNT, RESET_SELL_PRICE_EVERY) => {
...
const updateStateToReadyToBuyShort = (state, TRAILING_BUY_AMOUNT) => {
    ...
    state.trailingBuyPriceShort = Number(state.highestPriceAfterSell - TRAILING_BUY_AMOUNT)
}
...
const updateStateAfterMisBuyShort = (state, TRAILING_BUY_AMOUNT, RESET_SELL_PRICE_EVERY) => {
...
const updateStateToReadyToSellLong = (state, TRAILING_STOP_AMOUNT) => {
    ...
    state.trailingSellPriceLong = state.highestPriceAfterBuy - TRAILING_STOP_AMOUNT
}

const updateStateToReadyToSellShort = (state, TRAILING_STOP_AMOUNT) => {
    state.lowestPriceAfterBuy = state.currentPrice
    state.trailingSellPriceShort = state.lowestPriceAfterBuy + TRAILING_STOP_AMOUNT
}
